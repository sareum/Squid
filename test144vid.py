...
if __name__ == "__main__":
    ...
    phase_increment = 0.25  # Increment phase offset by 0.5 every 10 seconds
    phase_max = 2  # Maximum phase offset
    num_cycles = 50  # Total number of cycles
    cycle_time = T_right * num_cycles  # Total time for all cycles
    ...
    try:
        while True:
            ...
            # Break the loop after completing the designated cycles
            if t >= cycle_time:
                print("Completed 12 cycles.")
                break

            # Update the phase offset for the left motors every 10 seconds
            if int(t) % 5 == 0:  # Check if 5 seconds have passed
